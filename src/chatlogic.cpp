...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
  	...
  	ChatBot _chatBotToMove("../images/chatbot.png");
  
  
  	SetChatbotHandle(&_chatBotToMove);

    
    _chatBotToMove.SetChatLogicHandle(this);
    _chatBotToMove.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(_chatBotToMove));
